    ...
    Tcl_Interp *interp)		/* Interpreter to initialize. */
{
     ...
     *				  an init.tcl file.
     *
     *	$env(ATL_LIBRARY)	- highest priority so user can always override
...
"    } else {\n"
"	set scripts {}\n"
"	if {[info exists env(ATL_LIBRARY)] && ($env(ATL_LIBRARY) ne {})} {\n"
"	    lappend scripts {set env(ATL_LIBRARY)}\n"
"	    lappend scripts {\n"
"if {[regexp ^tcl(.*)$ [file tail $env(ATL_LIBRARY)] -> tail] == 0} continue\n"
"if {$tail eq [info tclversion]} continue\n"
"file join [file dirname $env(ATL_LIBRARY)] tcl[info tclversion]}\n"
"	}\n"
