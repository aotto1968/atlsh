...
static char defaultLibraryDir[sizeof(ATL_LIBRARY)+200] = ATL_LIBRARY;
...
#define LIBRARY_SIZE	    32
    ...
    /*
     * Look for the library relative to the ATL_LIBRARY env variable. If the
     * last dirname in the ATL_LIBRARY path does not match the last dirname in
     * the installLib variable, use the last dir name of installLib in
     * addition to the orginal ATL_LIBRARY path.
     */

    str = getenv("ATL_LIBRARY");			/* INTL: Native. */
	...
	/*
	 * If ATL_LIBRARY is set, search there.
	 */
	...
	if ((pathc > 0) && (strcasecmp(installLib + 4, pathv[pathc-1]) != 0)) {
	    /*
	     * If ATL_LIBRARY is set but refers to a different tcl
