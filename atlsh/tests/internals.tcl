...
if {[namespace which -command ::atltest::internals::scriptpath] eq ""} {namespace eval ::atltest::internals {

namespace path ::atltest

::atltest::ConstraintInitializer testWithLimit { expr {[testConstraint macOrUnix] && ![catch { exec prlimit --version }]} }
...
proc testWithLimit args {
	...
	atltest::Warn "testWithLimit: error - [regsub {^\s*([^\n]*).*$} $msg {\1}]"
	atltest::Skip testWithLimit
    }
	...
	atltest::Warn "testWithLimit: wrong limit, result: $result"
	atltest::Skip testWithLimit
    }
    return {*}$opt $result
}
